...
<body>
    ...
    <h1 class="text-center mt-4">Detail Job <?php echo $Section; ?></h1>



<form action="receive_sub.php" method="post">
    <div class="container table">
        <table id="example" class="table table-striped table-bordered" width="100%">
            <div class="d-flex justify-content-end">
                <a href="./<?php echo $Department.'/'.$Section.'.php'; ?>" class="btn btn-secondary btn-sm m-1">Back</a>
                <button type="submit" class="btn btn-primary btn-sm m-1" name="Receive">Receive</button>
            </div>
            ...
            <tbody>
                ...
                while (@$row = @$result->fetch(PDO::FETCH_ASSOC)) { 
                    $finishValues = @$row['Finish'];
                            if ($finishValues === "N") {

                                $detail = "finish_sub.php?ID=" . $row['ID'] . "&Department=" . $row['Department'] . "&Section=" . $row['Section']. "&Subject=" . $row['Subject'];
                                $btn = "btn-warning";
                            } else {
                                $detail = "#";
                                $btn = "btn-success";
                            }
                    $finishValue = @$row['Receive'];
                    ...
                    <tr>
                        <td><input style="width: 25px; height: 25px" type="checkbox" class="checkbox " name="ids[]" value="<?php echo @$row["ID"] ?>" <?php echo $isDisabled; ?> require></td>
